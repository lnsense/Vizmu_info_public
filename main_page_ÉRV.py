...
def find_matches(kerdes, keresesek_szama, collection_name):
    ...
    if collection_name == "ERV_tajekoztato": # ezt majd törölni kell ha lesz több doksi a tájékoztatós adatbázisban
        keresesek_szama = 25
...
##
##    if collection_name == "ERV_tartalomjegyzek_szabalyzat":        
...
def find_last_page_number_tajekoztato(id_number):
    ...
    collection = client.get_collection(name="ERV_tajekoztato")
...
##        id_data = "id1 - id394: Üzletszabályzat, id395 - id418: Árnyilvántartás"    
    ...
    try:
        assistant = AI_client.chat.completions.create(            
            ...
            messages=[
            {"role": "system", "content": """A ÉRV Zrt. víziközmű szolgáltató cég virtuális asszisztense vagy.
                                            ...
                                            """},
            {"role": "user", "content": f"A ÉRV Zrt. dolgozójának kérdése: {kerdes} \nRendelkezésre álló adatok: {merged_data}"},
          ]
        )
...
def store_data_in_excel_szabalyzat(kerdes_szabalyzat, valasz_rag1_szabalyzat, valasz_rag2_szabalyzat, koltseg_rag1_szabalyzat, koltseg_rag2_szabalyzat, model, username):
    # Save data to Google Sheets (worksheet: "ERV_Szabályzat") via streamlit_gsheets
    try:
        conn = st.connection("gsheets_kereso", type=GSheetsConnection)
        df = conn.read(worksheet="ERV_Szabályzat", ttl=0)
    except Exception as e:
    ...
    try:
        ...
        conn.update(worksheet="ERV_Szabályzat", data=updated)
...
def store_data_in_excel_jogszabaly(kerdes_jogszabaly, valasz_rag1_jogszabaly, valasz_rag2_jogszabaly, koltseg_rag1_jogszabaly, koltseg_rag2_jogszabaly, model, username):
    # Save data to Google Sheets (worksheet: "ERV_Jogszabály") via streamlit_gsheets
    try:
        conn = st.connection("gsheets_kereso", type=GSheetsConnection)
        df = conn.read(worksheet="ERV_Jogszabály", ttl=0)
    except Exception as e:
        st.error(f"Nem sikerült olvasni a Google Sheet-et: {e}")
        return
    ...
    try:
        updated = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        conn.update(worksheet="ERV_Jogszabály", data=updated)
        print("Adatok lementve Google Sheet-be.")
    except Exception as e:
        st.error(f"Nem sikerült írni a Google Sheet-be: {e}")
...
def store_data_in_excel_tajekoztato(kerdes_tajekoztato, valasz_rag1_tajekoztato, valasz_rag2_tajekoztato, koltseg_rag1_tajekoztato, koltseg_rag2_tajekoztato, model, username):
    # Save data to Google Sheets (worksheet: "ERV_Tájékoztató") via streamlit_gsheets
    try:
        conn = st.connection("gsheets_kereso", type=GSheetsConnection)
        df = conn.read(worksheet="ERV_Tájékoztató", ttl=0)
    except Exception as e:
        st.error(f"Nem sikerült olvasni a Google Sheet-et: {e}")
        return
    ...
    try:
        updated = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        conn.update(worksheet="ERV_Tájékoztató", data=updated)
        print("Adatok lementve Google Sheet-be.")
    except Exception as e:
        st.error(f"Nem sikerült írni a Google Sheet-be: {e}")
...
def add_rag1_feedback_to_excel(valasz_rag1, valasz_rag1_visszajelzes, valasz_rag1_helytelen_user_input = None):
    try:
        conn = st.connection("gsheets_kereso", type=GSheetsConnection)
        sheet_name = "ERV_Szabályzat" if selected_tab == "Szabályzat" else (
            "ERV_Jogszabály" if selected_tab == "Jogszabály" else "ERV_Tájékoztató"
        )
...
def add_rag2_feedback_to_excel(valasz_rag2, valasz_rag2_visszajelzes, valasz_rag2_helytelen_user_input = None):
    try:
        ...
        if valasz_rag2 == st.session_state.get('valasz_rag2_szabalyzat'):
            sheet_name = "ERV_Szabályzat"
        elif valasz_rag2 == st.session_state.get('valasz_rag2_jogszabaly'):
            sheet_name = "ERV_Jogszabály"
        else:
            sheet_name = "ERV_Tájékoztató"
        df = conn.read(worksheet=sheet_name, ttl=0)
    except Exception as e:
        st.error(f"Nem sikerült olvasni a Google Sheet-et: {e}")
        return
...
with col2:
    ...
    st.markdown(
        "<h1 style='text-align: center;'>ÉRV Zrt. Információ Kereső</h1>",
        unsafe_allow_html=True
    )
...
##                content.empty()
            ...
            def find_and_store_toc_results_szabalyzat(user_input, collection_name):
                organized_matches_toc, valaszok_toc, eleresi_helyek_toc, ids_toc, oldalszamok_toc, nevek_toc = find_matches(user_input, keresesek_szama, collection_name="ERV_tartalomjegyzek_szabalyzat")
...
##                    st.empty()
                    if user_input_szabalyzat:          
                        ...
                        # Fetch results from "ERV_tartalomjegyzek_szabalyzat"
                        find_and_store_toc_results_szabalyzat(user_input = st.session_state['kerdes_szabalyzat'], collection_name="ERV_tartalomjegyzek_szabalyzat")
                        
                ##        if not st.session_state['valaszok_toc']:
                ##            st.error("Nem található válasz a megadott kérésre.")

         
            # Display buttons for "ERV_tartalomjegyzek_szabalyzat" matches if results exist
    ...
    ##                        st.empty()
                            ...
                            find_and_store_results_szabalyzat(user_input = st.session_state['kerdes_szabalyzat'], collection_name="ERV_szabalyzat")
            ...
            if st.session_state['kereses_popular_button_tajekoztato_pressed'] == False and st.session_state['kereses_button_tajekoztato'] == True:
                ...
                with st.spinner(f"Keresés: {st.session_state['kerdes_tajekoztato']}"):
                    ...
                    with st.container():
                        ...
                        find_and_store_results_tajekoztato(user_input = st.session_state['kerdes_tajekoztato'], collection_name="ERV_tajekoztato")
